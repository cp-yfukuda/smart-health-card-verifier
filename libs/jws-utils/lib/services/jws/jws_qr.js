...
export const getJWS = async (protocol, qr) => {
    const jwsString = chunksToJws(protocol, qr);
...
function chunksToJws(protocol, payloads) {
    ...
    if (jwsChunks
        ...
        .reduce((unbalanced, length) => unbalanced ||
        length < expectedChunkSize - balancedSizeBuffer ||
        length > expectedChunkSize + balancedSizeBuffer, false)) {
        console.log('QR chunk sizes are unbalanced: ' +
            jwsChunks.map((jwsChunk) => jwsChunk.length.toString()).join(), ErrorCode.UNBALANCED_QR_CHUNKS);
    }
    return jws;
}
function decodeToJws(protocol, jws, chunkCount = 1) {
    ...
    if (chunked && !isChunkedHeader) {
        ...
        const hasBadChunkCount = new RegExp(`^${qrHeader}${positiveIntRegExp}/[1-9][0-9]*/.*$`).test(jws);
        const found = jws.match(new RegExp(`^${qrHeader}${positiveIntRegExp}/(?<expectedChunkCount2>[1-9][0-9]*)/.*$`));
        if (found)
            console.log(`${found}`);
        if (hasBadChunkCount) {
    ...
    const validQrHeader = new RegExp(chunked ? `^${qrHeader}${positiveIntRegExp}/${chunkCount}/.*$` : `^${qrHeader}.*$`, 'g').test(jws);
    if (!validQrHeader) {
    ...
    const validQrEncoding = new RegExp(chunked ? `^${qrHeader}${positiveIntRegExp}/${chunkCount}/[0-9]+$` : `^${qrHeader}[0-9]+$`, 'g').test(jws);
    if (!validQrEncoding) {
