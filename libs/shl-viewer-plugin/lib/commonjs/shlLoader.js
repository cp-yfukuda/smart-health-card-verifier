...
var _SMARTDataLoader = _interopRequireDefault(require("./SMARTDataLoader"));
...
class SHLLoader {
  ...
  async validateSHL(shl) {
    ...
    if (shlResult) {
      ...
      if (this.isExpired()) {
        throw new _errors.SHLError(_errors.ErrorCode.SHL_EXPIRED);
      }
      if (this.needsPasscode() && passCode == null) {
        console.info("throwing error");
        throw new _errors.SHLError(_errors.ErrorCode.SHL_PASSCODE_NEEDED);
      }
      ...
      throw new _errors.SHLError(_errors.ErrorCode.JWS_RETRIEVAL_ERROR);
    }
  }
  ...
  async getSHLManifestFileInfo(passcode) {
    ...
    try {
      const response = await (0, _utils.fetchWithTimeout)(this.shlData.url, {
        method: 'POST',
        headers: {
      ...
      throw new _errors.SHLError(_errors.ErrorCode.SHL_MANIFEST_RETRIEVAL_ERROR);
    }
  }
  async fetchFHIRBundle(manifestFileInfo) {
    let dataLoader = await _SMARTDataLoader.default.init(this.shlData.key, manifestFileInfo);
  ...
  decodePayload(payload) {
    ...
    try {
      ...
      console.error(e);
      throw new Error(_errors.ErrorCode.SHL_DECODE_FAILED);
    }
  }
