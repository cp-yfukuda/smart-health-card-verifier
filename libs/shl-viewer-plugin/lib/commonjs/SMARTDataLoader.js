...
function _toPrimitive(input, hint) { if (typeof input !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (typeof res !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); } //import { KeysStore } from 'jws-utils'
...
class SMARTDataLoader {
  ...
  async load() {
    try {
      ...
      const keyBuffer = _nodeJose.default.util.base64url.decode(this.key);
      ...
      for (const i in shlFiles) {
        ...
        const ks = await _nodeJose.default.JWK.asKeyStore(keyStore);
        const decrypter = await _nodeJose.default.JWE.createDecrypt(ks);
  ...
  async fetchJWS(shlURL) {
    const responseRaw = await (0, _utils.fetchWithTimeout)(shlURL, {
      headers: {
        Origin: requestedOrigin
      }
    }, JWS_FETCH_TIMEOUT, "ERROR Fetching JWS").catch(_ => {
      throw new _errors.SHLError(_errors.ErrorCode.SHL_JWS_RETRIEVAL_ERROR);
    });
    return responseRaw;
  }
